...
fe_lag <- feols(
  poverty_rate_pct ~ minw_lag1 + minw_lag2 + z_share_lowedu + z_unemp + z_snap + z_share_nonwhite | STATEFIP + year,
  data = stag_did_fe2,
  cluster = ~ STATEFIP,
  weights = ~ pop_weight,
)
...
fig3 <- ggplot(plot_data, aes(x = as.numeric(as.character(year)), y = poverty_rate_pct, color = Group)) +
  ...
  labs(
    title = "Parallel Trends Check (Treated vs Control States)",
...
```{r}
p_es <- ggplot(es_df, aes(x = egt, y = att)) +
  geom_hline(yintercept = 0, color = "gray55", linewidth = 0.8, linetype = "dashed") +
  geom_linerange(aes(ymin = ci_low, ymax = ci_high), color = "gray50", linewidth = 0.7) +
  geom_line(aes(color = period), linewidth = 1.1) +
  geom_point(aes(color = period), size = 2.8) +
  scale_color_manual(values = c("Pre" = "gray50", "Post" = "red3")) +
  labs(
    title    = "Event Study – Minimum Wage & Poverty",
    x        = "Years since first increase",
    y        = "Average Treatment Effect on the Treated"

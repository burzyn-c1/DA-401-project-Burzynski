...
ggsave("timing_of_treatment.png", timing_mw, width = 6, height = 4.5, dpi = 300)
...
p_dyn <- ggdid(agg_dynamic_cov) +
  labs(
    ...
    y = "Average Treatment Effect on the Treated (% points)"
  ) +
...
```{r}
#this was made earlier originally, just polished down here
...
p_exp <- ggdid(agg_dynamic_cov) +
  labs(
    title    = "Average Effect by Length of Exposure on the Treatment Group",
...
```{r}
#configuring tables and figures for paper that hasn't been done prior. 


coef_labels <- c(
  "log_minw_real"     = "Log real minimum wage",
  "z_share_lowedu"    = "Share with less than high school (standardized)",
  "z_unemp"           = "Unemployment rate (standardized)",
  "z_snap"            = "Share receiving SNAP (standardized)",
  "z_share_nonwhite"  = "Share nonwhite (standardized)"
)
fixest_to_flextable <- function(model, coef_labels, title = NULL, digits = 2) {
  ...
  out <- ct %>%
    transmute(
      Term = recode(term, !!!coef_labels, .default = term),
...
tab1_ft  <- fixest_to_flextable(fe_model_cov, coef_labels,
                                title = "Table 1. Covariate-adjusted two-way fixed effects estimates")

tabA1_ft <- fixest_to_flextable(fe_model, coef_labels,
                                title = "Appendix Table A1. Baseline two-way fixed effects estimates")
...
aggte_to_df <- function(agg) {
  ...
  if (!is.null(agg$att.egt) && !is.null(agg$se.egt) && !is.null(agg$egt)) {
    cohort <- data.frame(
      Group  = paste0("First treated in ", agg$egt),
      ATT    = agg$att.egt,
      SE     = agg$se.egt,
      CI_low = agg$att.egt - 1.96 * agg$se.egt,
      CI_high= agg$att.egt + 1.96 * agg$se.egt
    )
    out <- bind_rows(overall, cohort)
  } else {
    out <- overall
  }

  out %>% mutate(across(c(ATT, SE, CI_low, CI_high), ~ round(.x, 2)))
}

```

```{r}
tab2_df  <- aggte_to_df(agg_overall_cov)
tabA2_df <- aggte_to_df(agg_overall)

names(tab2_df)  <- c("Group", "ATT (percentage points)", "Std. error", "95% CI (low)", "95% CI (high)")
names(tabA2_df) <- c("Group", "ATT (percentage points)", "Std. error", "95% CI (low)", "95% CI (high)")

tab2_ft <- flextable(tab2_df) %>%
  set_caption("Table 2. Covariate-adjusted staggered DiD estimates (aggregated ATT)") %>%
  autofit()

tabA2_ft <- flextable(tabA2_df) %>%
  set_caption("Appendix Table A2. Baseline staggered DiD estimates (aggregated ATT)") %>%
  autofit()
...
summary_df <- stag_did_fe2 %>%
  select(any_of(vars_for_summary)) %>%
  summarise(across(
    everything(),
    list(
...
var_labels <- c(
  "poverty_rate_pct" = "Poverty rate among full-time low-wage workers (percent)",
  "min_wage_real"    = "Real minimum wage",
  "log_minw_real"    = "Log real minimum wage",
  "annual_unemp_sa"  = "Unemployment rate (annual, percent)",
  "share_foodstmp"   = "Share receiving SNAP",
  "share_lowedu"     = "Share with less than high school",
  "share_nonwhite"   = "Share nonwhite"
)

summary_df$Variable <- recode(summary_df$Variable, !!!var_labels)

summary_df <- summary_df %>%
  mutate(across(where(is.numeric), ~ round(.x, 2)))

tabA3_ft <- flextable::flextable(summary_df) %>%
  flextable::autofit()
tabA3_ft
```
...
doc <- doc %>%
  ...
  body_add_flextable(tab2_ft) %>%
  body_add_par("", style = "Normal") %>%
  body_add_flextable(tabA1_ft) %>%
  body_add_par("", style = "Normal") %>%
  body_add_flextable(tabA2_ft) %>%
  body_add_par("", style = "Normal") %>%
  body_add_flextable(tabA3_ft)

print(doc, target = "tables_for_paper.docx")

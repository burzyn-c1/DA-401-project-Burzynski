...
panel_clean2$STATEFIP <- as.factor(as.numeric(panel_clean2$STATEFIP))
...
by_state <- panel_clean2 %>%
  group_by(STATEFIP) %>%
  summarise(
    binding_2010 = any(binding_nominal & year == 2010),
    ever_binding = any(binding_nominal, na.rm = T),
    ever_unbound = any(!binding_nominal, na.rm = T),
    first_treat = ifelse(ever_binding & ever_unbound, 
                         min(year[binding_nominal], na.rm = T),
                         NA_integer_)
  )
...
```{r}
always_treated <- by_state %>%
  filter(binding_2010 & !ever_unbound) %>%
  pull(STATEFIP)

never_treated <- by_state %>%
  filter(!ever_binding) %>%
  pull(STATEFIP)

adopters <- by_state %>%
  filter(!binding_2010 & ever_binding) %>%
  pull(STATEFIP)
  
  
by_state %>%
  summarise(
    always_treated = sum(binding_2010 & !ever_unbound),
    never_treated = sum(!ever_binding), 
    adopters = sum(!binding_2010 & ever_binding)
  )
```

```{r}
#merge first treatment back to main panel
panel_did <- panel_clean2 %>%
  left_join(by_state %>% select(STATEFIP, first_treat), by = "STATEFIP") 

#filter for staggered did
stag_did <- panel_did %>%
  filter(!STATEFIP %in% always_treated) #removed always treated; states with no pre-period as minimum wage was always above 7.25
```

```{r}
#Timing of First minimum wage increases above 7.25 by year
by_state %>%
  filter(!STATEFIP %in% always_treated, !is.na(first_treat)) %>%
...
did_out <- att_gt(
  ...
  idname = "STATEFIP", 
  gname = "first_treat", 
  xformla = ~ log_income + log_gdp + annual_unemp_sa,
  data = panel_clean2,

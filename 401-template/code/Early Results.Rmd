...
stag_est_win <- stag_est %>%
  ...
  filter(is.na(event_time) | (event_time >= -4 & event_time <= 7))
...
p_dyn <- ggdid(agg_dyn) +
  labs(
    ...
    subtitle = "Callaway & Sant’Anna staggered DID (no controls)",
    x        = "Years since first increase",
    y        = "ATT (percentage points)",
...
p_exp <- ggdid(agg_dynamic_cov) +
  labs(
    ...
    subtitle = "ATT by exposure year relative to first minimum wage increase (with controls)",

...
stag_est_win <- stag_est_win %>%
  ...
  drop_na(poverty_rate_pct, z_med_income, z_snap, z_share_lowedu, z_share_nonwhite, z_share_under30)
...
summary(stag_est_win)
table(stag_est_win$first_treat)
```
...
covars <- stag_est_win %>%
  select(z_med_income, z_snap, z_share_lowedu, z_share_nonwhite, z_share_under30, poverty_rate_pct)
...
att_cov_nyt <- att_gt(
  ...
  idname = "STATEFIPS",
  gname = "first_treat",
  xformla = ~ z_share_lowedu + z_snap + z_med_income + z_share_nonwhite + z_med_income,
  data = stag_est_win,
  panel = F,
...
fig3 <- ggplot(plot_data, aes(x = as.numeric(as.character(year)), y = poverty_rate_pct, color = Group)) +
  ...
  stat_summary(fun = "mean", geom = "point", size = 2.5) +
  labs(
    title = "Figure 3: Parallel Trends Check (Treated vs Control States)",
    subtitle = "Average poverty rates for full-time low-wage workers",
    x = "Year",
    y = "Mean Poverty Rate (%)",
    caption = "Notes: 'Treated' states raised their minimum wage above $7.25; 'Control' states never did."
  ) +
  custom_theme +
  theme(legend.position = "bottom")

ggsave("fig3_parallel_trends.png", fig3, width = 6.5, height = 4.5, dpi = 300)
...
```{r}
# Figure 5: Event Study – Minimum Wage → Poverty
p_dyn <- ggdid(agg_dynamic_cov) +
  labs(
    title    = "Figure 5: Event Study – Minimum Wage → Poverty",
    subtitle = "Callaway & Sant’Anna staggered DID (with controls)",
    x        = "Years since first increase",
    y        = "ATT (percentage points)",
    caption  = "Notes: Points show group-time ATT relative to first increase; vertical bars represent 95% CIs. Pre-treatment periods in red; post-treatment in blue."
  ) +
  custom_theme


# Save high-res
ggsave("fig5_event_study.png", p_dyn, width = 7, height = 5, dpi = 300)
...
```{r}
p_exp <- ggdid(agg_dynamic_nyt) +
  labs(
    title    = "Figure 6: Average Effect by Length of Exposure",
    subtitle = "ATT by exposure year relative to first minimum wage increase (not-yet-treated controls)",
    x        = "Exposure year (relative to first increase)",
    y        = "ATT (percentage points)",
    caption  = "Notes: Year 0 marks the first increase; vertical bars represent 95% CIs. Pre-treatment periods in red; post-treatment in blue."
  ) +
  custom_theme


# Save high-res
ggsave("fig6_avg_effect_exposure.png", p_exp, width = 7, height = 5, dpi = 300)

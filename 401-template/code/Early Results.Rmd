...
p_dyn <- ggdid(agg_dyn) +
  labs(
    title    = "Figure 1: Event Study – Minimum Wage & Poverty",
    subtitle = "Callaway & Sant’Anna staggered DID (no controls)",
    x        = "Years since first increase",
    y        = "ATT (percentage points)",
    caption  = "Notes: Points show group-time ATT relative to first increase; vertical bars represent 95% CIs. \nPre-treatment periods in red; post-treatment in blue."
  ) +
  custom_theme


ggsave("fig1_event_study.png", p_dyn, width = 7, height = 5, dpi = 300)
...
p_exp <- ggdid(agg_dynamic_cov) +
  labs(
    ...
    caption  = "Each point shows how poverty among full time low wage workers changed in states that raised their minimum wage, compared with states \nthat never did. Values to the left of zero represent years before the first increase and remain close to zero, which suggests that \nstates followed similar trends before the policy took effect. Beginning at year zero, when the minimum wage increases, the estimates \nbecome progressively more negative, showing that poverty declines over time. By two to five years after the increase, states display sizable \nreductions in poverty for full time low wage workers. Vertical bars represent 95 percent confidence intervals."
  ) +
  custom_theme
